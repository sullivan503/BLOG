...
const Essays: React.FC<EssaysProps> = ({ posts, onNavigate, categorySlug }) => {
  ...
  const isSpecialContent = (post: BlogPost) => {
    const hasSpecialTag = post.tags.some(t => 
      ['book', 'reading', 'model', 'project', 'achievement', '书', '模型', '项目', '成果'].some(keyword => t.toLowerCase().includes(keyword))
    );
    const hasSpecialCategory = post.categories?.some(c => {
        const lower = c.toLowerCase();
        return (
            lower === 'books' || c === '书架' || 
            lower === 'models' || c === '思维模型' || 
            lower === 'projects' || c === '咨询与案例' ||
            lower === 'achievements' || c === '精选成果'
        );
    });

    return hasSpecialTag || hasSpecialCategory;
  };

  // Filter Logic: Category -> then Search
  const filteredPosts = useMemo(() => {
    let results = posts.filter(post => !isSpecialContent(post));
    ...
    // 2. Filter by Search Query (Only applies to main view if search input is present, but kept safe here)
  ...
  const categories = useMemo(() => {
    ...
    const validPosts = posts.filter(post => !isSpecialContent(post));
